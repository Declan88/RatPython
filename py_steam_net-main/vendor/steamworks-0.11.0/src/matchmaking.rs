...
impl<Manager> Matchmaking<Manager> {
    ...
    pub fn add_request_lobby_list_string_filter(
        ...
        // PATCHED (see py_steam_net's own Cargo.toml [patch.crates-io]
        // comment for why this vendored copy exists at all): the
        // upstream crate passed key.as_ptr()/value.as_ptr() straight
        // from a Rust &str, which has NO null terminator, directly to
        // a C API that reads a C string until it hits one - undefined
        // behavior that in practice made every filtered search silently
        // match nothing (confirmed empirically: identical unfiltered
        // RequestLobbyList found the exact lobby a filtered one
        // couldn't). set_lobby_data/delete_lobby_data elsewhere in this
        // same file already do this correctly via CString::new(...) -
        // this brings the filter functions in line with that.
        let key = CString::new(key).unwrap();
        let value = CString::new(value).unwrap();
        unsafe {
    ...
    pub fn add_request_lobby_list_numerical_filter(
        ...
        // PATCHED - same non-null-terminated-&str bug as
        // add_request_lobby_list_string_filter above, same fix.
        let key = CString::new(key).unwrap();
        unsafe {
